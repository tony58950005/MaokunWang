...
bool HighLevelComm::ParseMessage(uint8_t message[])	//TODO-Akos: Rename this function to ParseMessage. Create a private class member variable from myRxData_9bits, and also check its type (currently it is one byte).
{
	if (uart.receiveMessage(message, sizeof(message), 100) == true){
	 for(int i=0; i<sizeof(message); i++)
	 {
		 myRxData_9bits[i] = message[i];
